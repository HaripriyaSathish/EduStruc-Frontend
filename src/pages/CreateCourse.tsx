...
export default function CreateCourse() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      // Build semester/schedule string
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#EFF4FF', fontFamily: 'Inter, sans-serif' }}>
      ...
      <style>{`
        ...
        .create-btn:disabled { background: #93B4E8 !important; cursor: not-allowed !important; }
        ...
        <main style={{ padding: '28px 32px', flex: 1 }}>
          <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
            ...
            <div style={{ marginBottom: '24px' }}>
              <h1 style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 700, fontSize: '22px', color: '#0B1C30', margin: '0 0 4px' }}>Create New Course</h1>
              <p style={{ fontSize: '13px', color: '#76777D', margin: 0 }}>Define administrative and academic parameters for the upcoming semester.</p>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
              ...
              <div style={{ background: '#fff', border: '1px solid #C6C6CD', borderRadius: '12px', padding: '24px' }}>
                ...
                <div style={{ marginBottom: '16px' }}>
                  <label style={labelStyle}>Assign Real Teacher <span style={{ color: '#76777D', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(optional — links to Teacher Timetable)</span></label>
                  <select className="form-input" style={inputStyle} value={teacherId}
                    onChange={e => {
                      setTeacherId(e.target.value);
                      const t = teachers.find(t => String(t.id) === e.target.value);
                      if (t) setInstructor(t.full_name);
                    }}>
                    <option value="">Not linked — use text field below</option>
              ...
              <div style={{ background: '#fff', border: '1px solid #C6C6CD', borderRadius: '12px', padding: '24px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                  <div>
                    ...
                    <select className="form-input" style={inputStyle} value={semester} onChange={e => setSemester(e.target.value)}>
                      {['Fall 2024','Spring 2025','Summer 2025','Fall 2025'].map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                  </div>
                  <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              <button className="cancel-btn" onClick={() => navigate('/courses')}
