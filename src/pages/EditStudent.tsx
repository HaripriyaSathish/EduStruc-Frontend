  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        const msg = data.email?.[0] || data.roll_number?.[0] || 'Update failed. Please check the form.';
        setError(msg);
      }
    } catch {
      ...
      <div style={{ marginLeft: '240px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <header style={{ background: '#fff', borderBottom: '1px solid #C6C6CD', height: '64px', padding: '0 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 30 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontSize: '13px', color: '#76777D', cursor: 'pointer' }} onClick={() => navigate(`/students/${id}`)}>
        ...
        <main style={{ padding: '28px 32px', flex: 1 }}>
          ...
          {loading && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '300px', gap: '12px', flexDirection: 'column' }}>
              ...
              <p style={{ color: '#76777D', fontSize: '14px' }}>Loading student data...</p>
            </div>
          )}
            ...
            <div style={{ maxWidth: '760px', margin: '0 auto' }}>
              ...
              <div style={{ background: '#fff', border: '1px solid #C6C6CD', borderRadius: '12px', padding: '28px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', paddingTop: '16px', borderTop: '1px solid #F0F4FF' }}>
                  <button className="cancel-btn" onClick={() => navigate(`/students/${id}`)}
